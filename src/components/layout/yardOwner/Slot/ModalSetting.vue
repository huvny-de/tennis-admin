  ...
  <div id="modal" class="flex modal fixed inset-0" :class="isClose === true ? 'hidden' : ''">
    ...
    <div class="mx-auto relative p-4 w-full max-w-5xl md:h-auto">
      <div class="relative h-[80%] bg-white rounded-lg shadow dark:bg-gray-700 mt-12">
        <button @click="hiddenModal" type="button" class="
            absolute
            top-3
            right-2.5
            text-gray-400
          ...
          <div class="h-96 bg-red-5 overflow-y-auto">
            <form action="#">
              <table class="
                ...
                <tbody>
                  <tr class="
                      ...
                      odd:bg-white
                      even:bg-gray-50
                    " v-for="slot in slot.slotDetail" :key="slot">
                    <td class="px-2 py-4">
                      <div class="flex slot-center">
                        <div class="ml-4 text-[#334D6E]">{{ slot.no }}</div>
                      </div>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4">
                      <input type="number" class="
                        ...
                        " id="username" placeholder="Giá Tiền" v-model="slot.price" />
                    </td>
                  </tr>
                </tbody>
              </table>
              ...
              <div class="flex justify-end pt-4 px-8">
                ...
                <button type="button" @click="settingPrice" class="
                    modal-close
                    px-4
                    bg-indigo-500
                    p-1
...
export default {
  ...
  methods: {
    ...
    settingPrice() {
      ...
      this.slot.slotDetail.forEach((slot) => {
        slot.applyTypeId = this.detail.id;
      })
      ...
      if (!setting_all_slot) {
        ...
        sessionStorage.setItem('setting_all_slot', JSON.stringify(new_array_setting))
      } else {
        ...
        sessionStorage.setItem('setting_all_slot', JSON.stringify(setting_all_slot))
      }
  ...
  watch: {
    ...
    idType() {
      ...
      if (!this.detail) {
        let setting_all_slot = JSON.parse(sessionStorage.getItem('setting_all_slot'));
        if (setting_all_slot) {
          this.setting_all_slot.forEach((slot) => {
            if (slot.id == this.idCourtType) {
              console.log(slot)
              this.slot = slot
            }
          })
