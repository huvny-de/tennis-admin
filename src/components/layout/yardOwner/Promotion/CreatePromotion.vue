...
export default {
  ...
  mounted() {
    if (!this.promotion.imgUrl) {
      this.promotion.imgUrl = "https://i.ibb.co/8Psbxxv/download.png";
    }
  },
  data() {
    return {
  ...
  methods: {
    uploadImg(evt) {
      ...
      axios
        ...
        .then((res) => {
          this.promotion.imgUrl = res.data.data.image.url;
        })
        .catch((err) => {
          console.log(err);
          this.loading = false;
